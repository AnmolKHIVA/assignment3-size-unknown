...
ANSWER:
We can search an element in an array whose size is unknown in O(logn) time complexity provided that the array is in sorted order (here i am taking case of ascending order).We can do this by making some modification in binary search technique.
...
int binary_search(int arr[],int i,int j,int key){
    ...
    return -1;                         // return -1  when element is not present 
}
...
int binary_search_unknown(int arr[],int key){
    ...
    // Call Simple Binary Search on range i to j.
